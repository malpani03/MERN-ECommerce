...
const PendingOrders = () => {
  ...
  useEffect(() => {
    const fetchPendingOrders = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/pendingorders',{
            credentials: 'include'
        });
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/orders/updateStatus`, {
        ...
        body: JSON.stringify({ orderId, status: newStatus }) // Include orderId in the payload
      });
      const data = await response.json();
      if (data.success) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-semibold mb-8">Pending Orders</h1>
      {selectedOrder ? (
        <div>
